    ...
    new_query = <<~SQL
      CREATE MATERIALIZED VIEW dashboard_stats AS (
        SELECT
          ...
          (SELECT COUNT(DISTINCT ads.id) FROM ads JOIN user_contacts ON ads.phone_number_id = user_contacts.phone_number_id WHERE deleted = FALSE) AS effective_ads_count,
          ...
          (SELECT created_at FROM ads WHERE id = (SELECT MAX(ads.id) FROM ads JOIN user_contacts ON ads.phone_number_id = user_contacts.phone_number_id WHERE deleted = FALSE) LIMIT 1) AS last_effective_ad_created_at,
          (

  ...
  module V2
    class FeedAdsController < ApplicationController
      ...
      def index
        ...
        ads = Ad.where(id: ads.map(&:id)).eager_load(:category, :ad_images, :ad_description, :ad_extra, :ad_query, :ad_image_links_set, :city, :region, :ad_favorites).order('ads.id DESC')

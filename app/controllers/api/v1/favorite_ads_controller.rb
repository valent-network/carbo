...
module Api
  module V1
    class FavoriteAdsController < ApplicationController
      ...
      def index
        ads = Ad.includes(:ad_description, :ad_image_links_set, city: [:region], ad_options: [:ad_option_type, :ad_option_value]).joins(:ad_favorites).where(ad_favorites: { user_id: current_user.id }).order('ads.created_at DESC').limit(20).offset(params[:offset])

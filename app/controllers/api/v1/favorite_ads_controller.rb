...
module Api
  module V1
    class FavoriteAdsController < ApplicationController
      ...
      def index
        ads = Ad.eager_load(:ad_description, :ad_extra, :ad_query, :ad_image_links_set, :city, :region).joins(:ad_favorites).where(ad_favorites: { user_id: current_user.id }).order('ads.created_at DESC').limit(20).offset(params[:offset])

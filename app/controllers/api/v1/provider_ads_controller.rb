...
module Api
  module V1
    class ProviderAdsController < ApplicationController
      ...
      def update_ad
        ad = Ad.where(address: ad_params[:details][:address], ads_source_id: current_ads_source.id).first_or_initialize(stale: false)
      ...
      def ad_params
        details_params = %i[maker model race address new_car customs_clear year images_json_array_tmp engine_capacity fuel horse_powers gear wheels carcass color description state_num]

...
class UserConnection < ApplicationRecord
  ...
  scope :known_contacts_count, -> { select('user_connections.user_id, COUNT(DISTINCT "user_contacts"."phone_number_id") AS count').left_joins(:user_contacts).group('user_connections.user_id') }

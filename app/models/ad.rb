...
class Ad < ApplicationRecord
  ...
  scope :order_by_fav_for, ->(user) { Ad.joins(:ad_favorites).from("(#{select('id, MAX(created_at) AS created_at').from("(#{user.events.ad_favorites_ordered.to_sql}) AS ads").group('id').to_sql}) AS ads").order('created_at DESC') }
